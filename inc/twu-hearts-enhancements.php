...
class Hearts_Theme_Plugin_Enhancements {
	...
	static function init() {
		...
		if ( ! $instance ) {
			$instance = new Hearts_Theme_Plugin_Enhancements;
		}
	...
	function admin_notices() {
		...
		if ( get_user_meta( get_current_user_id(), 'twu_hearts_admin_notice', true ) === 'dismissed' ) {
			return;
		}
		...
		printf(
			'<div id="twu-hearts-notice" class="notice notice-warning is-dismissible">%s</div>',
...
add_action( 'admin_head', array( 'Hearts_Theme_Plugin_Enhancements', 'init' ) );

function twu_hearts_enqueue_scripts() {
	// Add the admin JS if the notice has not been dismissed
	if ( is_admin() && get_user_meta( get_current_user_id(), 'twu_hearts_admin_notice', true ) !== 'dismissed' ) {

		// Adds our JS file to the queue that WordPress will load
		wp_enqueue_script( 'twu_hearts_admin_script', get_stylesheet_directory_uri() . '/inc/twu-hearts-enhancements.js', array( 'jquery' ), '20180901', true );

		// Make some data available to our JS file
		wp_localize_script( 'twu_hearts_admin_script', 'twu_hearts_admin', array(
			'twu_hearts_admin_nonce' => wp_create_nonce( 'twu_hearts_admin_nonce' ),
		));
	}
}
add_action( 'admin_enqueue_scripts', 'twu_hearts_enqueue_scripts' );
 ...
 */
function twu_hearts_dismiss_admin_notice() {
	// Verify the security nonce and die if it fails
	if ( ! isset( $_POST['twu_hearts_admin_nonce'] ) || ! wp_verify_nonce( $_POST['twu_hearts_admin_nonce'], 'twu_hearts_admin_nonce' ) ) {
		wp_die( __( 'Your request failed permission check.', 'argent' ) );
	}
	// Store the user's dimissal so that the notice doesn't show again
	update_user_meta( get_current_user_id(), 'twu_hearts_admin_notice', 'dismissed' );
...
add_action( 'wp_ajax_twu_hearts_admin_notice', 'twu_hearts_dismiss_admin_notice' );
